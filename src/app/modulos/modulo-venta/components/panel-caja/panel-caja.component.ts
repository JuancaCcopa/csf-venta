...
onObtieneConfiguracionEstacionTrabajo(nombremaquina: string) {
  ...
  .subscribe((res: any[]) => {
      debugger
      console.log("onObtieneConfiguracionEstacionTrabajo",res);
      if(res.length>0){
        this.gEstacionTrabajo=res;
        this.metodosInicio();
        this.getCorrelativoConsulta(nombremaquina);
      }else{
        this.messageService.add({key: 'toasVentaCaja', severity:'error', summary: 'Mensaje', detail: ` NO TIENE CONFIGURADO LA ESTACION DE TRABAJO: ${nombremaquina}`});
      }
