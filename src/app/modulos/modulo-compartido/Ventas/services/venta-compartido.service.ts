...
import { ITabla } from '../interfaces/tabla.interface';
...
export class VentaCompartidoService {
  ...
  //JC
//getBancos
  getListaTablaLogistica(codtabla:string, buscar: string, key: number,numerolineas: number, orden: number) {
    let parametros = new HttpParams();
    ...
    // (`${environment.url_api_venta}Bancos/GetBancos`);
    return this.http.get<ITabla[]>
  ...
  // getTarjetacredito() {
  //   return this.http.get<ITabla[]>
  //   (`${environment.url_api_venta}TarjetaCredito/GetTarjetaCredito`);
  // }
  getTerminal() {
    return this.http.get<ITabla[]>
